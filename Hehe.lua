...
local function createTabContainer()
    ...
    sf.CanvasSize = UDim2.new(0, 0, 0, 380)
...
RayfieldBtn.MouseButton1Click:Connect(function()
    playClickSound()
    -- Xóa menu hiện tại của đồng chí để chuyển sang Rayfield chuẩn chỉnh
    gui:Destroy()
    
    -- Load thư viện Rayfield UI chính thức
    pcall(function()
        ...
        local Tab = Window:CreateTab("Slap Tower", 4483362458)
        Tab:CreateSection("Tính năng chính")
        Tab:CreateButton({
            Name = "Thông báo: Đã chuyển sang Rayfield!",
            Callback = function()
                Rayfield:Notify({
                    Title = "Thành công",
                    Content = "Giao diện Rayfield đã hoạt động!",
                    Duration = 4
                })
            end
        })
