...
-- DÒNG 3: ESP POLICE (Riêng biệt nhận diện cảnh sát)
...
-- DÒNG 4: KILL FRONT (Giết người đối diện khi làm Killer)
createMenuButton("Kill Front (Giết người đối diện)", Tab1Container, function(state)
    if state then
        pcall(function()
                    ...
                    if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        if distance <= 10 then
                            ...
                            if dotProduct > 0.4 then
                                local knife = char:FindFirstChildOfClass("Tool") or player.Backpack:FindFirstChildOfClass("Tool")
                                if knife then
                                    player.Character.Humanoid:EquipTool(knife)
                                    task.wait(0.1)
                                    targetRoot.CFrame = rootPart.CFrame + (rootPart.CFrame.LookVector * 2)
...
createMenuButton("Shot Killer (Police bắn Killer)", Tab1Container, function(state)
    if state then
        pcall(function()
            local char = player.Character
            if char and char:FindFirstChild("HumanoidRootPart") then
                ...
                for _, p in ipairs(Players:GetPlayers()) do
                    if p ~= player and p.Character then
                        ...
                        if isKiller and p.Character:FindFirstChild("HumanoidRootPart") then
                            local targetRoot = p.Character.HumanoidRootPart
                            local gun = char:FindFirstChildOfClass("Tool") or player.Backpack:FindFirstChildOfClass("Tool")
                            if gun then
                                player.Character.Humanoid:EquipTool(gun)
                                task.wait(0.1)
                                targetRoot.CFrame = rootPart.CFrame + (rootPart.CFrame.LookVector * 5)
                                p.Character.Humanoid.Health = 0
                            end
                        end
                    end
